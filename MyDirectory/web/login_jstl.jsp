...
<c:if test="${param['userID'] ne null}">
    <c:choose>
        <c:when test="${param['userID'] eq initParam.userid
                      && param['password'] eq initParam.password }">
            <c:set var="message" value="Success"></c:set>
            <jsp:setProperty name="user" property="userId" value="${param['userID']}"></jsp:setProperty>
