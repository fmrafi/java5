...
public class EntryControllerR extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        entry.setFirstName(request.getParameter("entry.firstName"));
        entry.setLastName(request.getParameter("entry.lastName"));
        try {
            DateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");
            entry.setDateOfBirth(formatter.parse(request.getParameter("entry.dateOfBirth")));
        } catch (ParseException ex) {
            Logger.getLogger(EntryControllerR.class.getName()).log(Level.SEVERE, null, ex);
        }
        entry.setPhoneNumber(request.getParameter("entry.phoneNumber"));
        ...
        for (DirectoryEntry d : directory.getEntries()) {
            if (d.getID().equals(id)) {
                d.setFirstName(entry.getFirstName());
                d.setLastName(entry.getLastName());
                d.setDateOfBirth(entry.getDateOfBirth());
                d.setPhoneNumber(entry.getPhoneNumber());
